...
# -*- coding: utf-8 -*-
"""
KLARTEXT ADHS-Deck – Kartengenerator, adaptiert von build_card_at.py. Fachprüfung durch externe
ADHS-Fachperson abgeschlossen (27.07.2026) – kein Entwurfs-/Fachprüfungs-Vorbehalt mehr.
Kind-facing wie JD/KD/AT (nicht Erwachsenen-Selbstreflexion wie EL/LK/TR): "TIPP FÜR DIE INGRA",
KEINE dritte systemische Frage. Anders als AT: normale, offene Fragen statt erzwungener wörtlicher
Sprache/geschlossener Fragen (ADHS betrifft Aufmerksamkeit/Impulskontrolle, nicht sprachliche
Verarbeitung im selben Sinn wie Autismus) – Struktur sonst identisch.
"""
...
# ── Farben (ADHS-Deckfarbe: Periwinkle #6B7FD7 – rechnerisch kollisionsgeprüft per RGB-Distanz
# gegen alle 21 bestehenden App-/Deck-/Modulfarben, größter Sicherheitsabstand aller geprüften
# Kandidaten, siehe ADHS_Kartenkonzept_Entwurf.md) ──
ADHS          = (107, 127, 215)   # #6B7FD7
ADHS_LIGHT    = (232, 234, 249)   # #E8EAF9
ADHS_BORDER   = (196, 203, 240)   # #C4CBF0
...
def build_front(card, image_path, out_path):
    ...
    if os.path.exists(image_path):
        ...
        d.rectangle((0, bild_top, CARD_W, bild_top + bild_h), fill=(228, 230, 240))

    tf_top = bild_top + bild_h
    d.rectangle((0, tf_top, CARD_W, CARD_H), fill=ADHS_LIGHT)
    d.line((0, tf_top, CARD_W, tf_top), fill=ADHS_BORDER, width=mm(0.5))
    f_titel = font(F_SERIF_BOLD, 5)
    lines = wrap_text(d, card["titel"], f_titel, CARD_W - mm(10))
    ly = tf_top + mm(4)
    for ln in lines[:2]:
        d.text((mm(5), ly), ln, font=f_titel, fill=ADHS)
        ly += mm(6.2)
...
def build_back(card, out_path):
    ...
    nr_text = card.get("id_text", f"ADHS-{card['nr']:02d}")
    ...
    if len(titel_lines) > 2:
        ...
        for ln in titel_lines[:2]:
            d.text((pad, ty), ln, font=f_rstitel, fill=ADHS)
            ty += mm(5.6)
        ...
        y = max(ty, y + mm(9))
    d.line((pad, y, CARD_W - pad, y), fill=ADHS_BORDER, width=mm(0.4))
    y += mm(3.3)

    f_label = font(F_SANS_BOLD, 2.4)
    f_anleitung = font(F_SERIF_ITALIC, 3.1)
    d.text((pad, y), "ANLEITUNG", font=f_label, fill=ADHS)
    y += mm(4.3)
    ...
    d.text((pad, y), "FRAGEN", font=f_label, fill=ADHS)
    y += mm(4.6)
    ...
    for frage in card["fragen"]:
        ...
        rounded_rect(d, (pad, y, CARD_W - pad, y + box_h), radius=mm(2), fill=ADHS_LIGHT)
        d.rectangle((pad, y, pad + mm(1), y + box_h), fill=ADHS)
        ty = y + box_pad_y
        for ln in lines:
    ...
    d.line((pad, foot_y, CARD_W - pad, foot_y), fill=ADHS_BORDER, width=mm(0.4))
    logo = logo_mark(5.5)
    img.paste(logo, (pad, foot_y + mm(2)), logo)
    f_foot = font(F_SANS_REG, 2.3)
    footer_deck = card.get("footer_deck", "ADHS-Deck")
