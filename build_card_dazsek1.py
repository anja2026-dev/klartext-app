...
# -*- coding: utf-8 -*-
"""
KLARTEXT DaZ-Sek-I-Deck (Deutsch als Zweitsprache, Sek I) – Kartengenerator, adaptiert von
build_card_dazgs.py. "TIPP FÜR DIE INGRA", KEINE dritte systemische Frage. Kein Fachprüfungs-
Vorbehalt (wie DaZ-GS). Kein Brainy (wie JD/AT/ADHS) – für Sek-I-Jugendliche zu kindlich.
"""
...
# ── Farben (DaZ-Sek-I-Deckfarbe: Bordeaux #6E1438 – rechnerisch kollisionsgeprüft per RGB-Distanz
# gegen alle 23 bestehenden App-/Deck-/Modulfarben (inkl. DaZ-GS-Azurblau), Abstand 70.2 zur
# naechsten Farbe (App-EL-Modul-Magenta). Bewusst dunkler/reifer als DaZ-GS, siehe
# DAZ-SEK1_Kartenkonzept_Entwurf.md) ──
DAZSEK1        = (110, 20, 56)     # #6E1438
DAZSEK1_LIGHT  = (240, 231, 235)   # #F0E7EB
DAZSEK1_BORDER = (208, 179, 191)   # #D0B3BF
...
def build_front(card, image_path, out_path):
    ...
    kopf_h = mm(11)
    d.rectangle((0, 0, CARD_W, kopf_h), fill=DAZSEK1)
    logo = logo_mark(7)
    img.paste(logo, (mm(5), (kopf_h - logo.height) // 2), logo)
    f_badge = font(F_SANS_BOLD, 2.6)
    badge_text = card.get("badge", "DAZ-SEK I · ANKOMMEN & IDENTITÄT")
    ...
    if os.path.exists(image_path):
        ...
        d.rectangle((0, bild_top, CARD_W, bild_top + bild_h), fill=(232, 214, 222))

    tf_top = bild_top + bild_h
    d.rectangle((0, tf_top, CARD_W, CARD_H), fill=DAZSEK1_LIGHT)
    d.line((0, tf_top, CARD_W, tf_top), fill=DAZSEK1_BORDER, width=mm(0.5))
    f_titel = font(F_SERIF_BOLD, 5)
    lines = wrap_text(d, card["titel"], f_titel, CARD_W - mm(10))
    ly = tf_top + mm(4)
    for ln in lines[:2]:
        d.text((mm(5), ly), ln, font=f_titel, fill=DAZSEK1)
        ly += mm(6.2)
...
def build_back(card, out_path):
    ...
    nr_text = card.get("id_text", f"DAZ-SEK1-{card['nr']:02d}")
    ...
    if len(titel_lines) > 2:
        ...
        for ln in titel_lines[:2]:
            d.text((pad, ty), ln, font=f_rstitel, fill=DAZSEK1)
            ty += mm(5.6)
        ...
        y = max(ty, y + mm(9))
    d.line((pad, y, CARD_W - pad, y), fill=DAZSEK1_BORDER, width=mm(0.4))
    y += mm(3.3)

    f_label = font(F_SANS_BOLD, 2.4)
    f_anleitung = font(F_SERIF_ITALIC, 3.1)
    d.text((pad, y), "ANLEITUNG", font=f_label, fill=DAZSEK1)
    y += mm(4.3)
    ...
    d.text((pad, y), "FRAGEN", font=f_label, fill=DAZSEK1)
    y += mm(4.6)
    ...
    for frage in card["fragen"]:
        ...
        rounded_rect(d, (pad, y, CARD_W - pad, y + box_h), radius=mm(2), fill=DAZSEK1_LIGHT)
        d.rectangle((pad, y, pad + mm(1), y + box_h), fill=DAZSEK1)
        ty = y + box_pad_y
        for ln in lines:
    ...
    d.line((pad, foot_y, CARD_W - pad, foot_y), fill=DAZSEK1_BORDER, width=mm(0.4))
    logo = logo_mark(5.5)
    img.paste(logo, (pad, foot_y + mm(2)), logo)
    f_foot = font(F_SANS_REG, 2.3)
    footer_deck = card.get("footer_deck", "DaZ-Sek-I-Deck")
