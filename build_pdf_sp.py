#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Baut die druckfertige PDF aus allen 7 SP-Deck-Karten."""
from PIL import Image, ImageDraw, ImageFont
import os, sys
Image.init()
sys.path.insert(0, os.path.dirname(__file__))
from build_booklet_sp import anleitung_seite1, quellen_seite1

KARTEN_DIR = "/sessions/kind-beautiful-ptolemy/mnt/outputs/sp_karten_komplett/"
OUT_PDF = "/sessions/kind-beautiful-ptolemy/mnt/outputs/KLARTEXT_SP-Deck_komplett.pdf"
...
def build_cover():
    ...
    d.text((mm(48), mm(34)), "Springer-INGRAs", font=f_sub, fill=(226, 218, 212))

    f_haupt = ImageFont.truetype(F_SERIF_BOLD, mm(34))
    d.text((mm(20), mm(90)), "SP-Deck", font=f_haupt, fill=SP)

    f_haupt2 = ImageFont.truetype(F_SANS_BOLD, mm(7))
    d.text((mm(20), mm(140)), "7 Karten für Springer-INGRAs", font=f_haupt2, fill=KT_INK)

    f_themen_label = ImageFont.truetype(F_SANS_BOLD, mm(5.5))
    d.text((mm(20), mm(155)), "GRUNDHALTUNG", font=f_themen_label, fill=SP)

    themen = ["Plug-and-Play statt langem Beziehungsaufbau",
              "Gast im System der Stammlehrkraft",
              "Chance des unbelasteten Neuanfangs",
              "Knappe, verlässliche Übergabe", "Selbstschutz bei Systemwechseln"]
    f_thema = ImageFont.truetype(F_SANS_REG, mm(5.4))
    row_h = mm(8.7)
    for i, t in enumerate(themen):
        x = mm(20)
        y = mm(164) + i * row_h
        d.ellipse((x, y + mm(2), x + mm(2.4), y + mm(4.4)), fill=SP)
        d.text((x + mm(5.5), y), t, font=f_thema, fill=KT_INK)

    box_y = mm(225)
    box_h = mm(22)
    d.rounded_rectangle((mm(20), box_y, W - mm(20), box_y + box_h), radius=mm(3), fill=(237, 230, 226))
    f_status_l = ImageFont.truetype(F_SANS_BOLD, mm(6))
    f_status = ImageFont.truetype(F_SANS_REG, mm(5.6))
    d.text((mm(28), box_y + mm(6)), "STAND DIESER AUSGABE", font=f_status_l, fill=SP)
    d.text((mm(28), box_y + mm(13)), "SP-Deck vollständig (7 Karten), fachlich geprüft.",
...
def run():
    pages = [build_cover(), anleitung_seite1(), quellen_seite1()]

    fehlt = []
    for nr in range(1, 8):
        vorn = os.path.join(KARTEN_DIR, f"SP-{nr:02d}_Vorderseite.png")
        hinten = os.path.join(KARTEN_DIR, f"SP-{nr:02d}_Rueckseite.png")
