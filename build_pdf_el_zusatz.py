#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Baut drei kompakte PDFs für die EL-Zusatzblöcke (Autismus, ADHS, Pflegekinder).
Jedes PDF: kurzes Deckblatt (Andockt-an, Hinweis) + 7 Karten (Vorder-/Rückseite).
Keine eigene Anleitung/Methodik/Glossar/Quellen — verweist auf die EL-Basis-Anleitung
(KLARTEXT_EL-Deck_Basis_komplett.pdf), da die Zusatzblöcke als Ergänzung dazu gedacht sind."""
from PIL import Image, ImageDraw, ImageFont
import os
Image.init()

KARTEN_DIR = "/sessions/kind-beautiful-ptolemy/mnt/outputs/el_zusatz_karten_komplett/"
...
def build_cover(titel, andockt_an, hinweis_text, stand_text):
    ...
    d.text((mm(46), mm(31)), "EL-Deck · Ergänzungsset", font=f_sub, fill=(250, 232, 224))

    y = mm(75)
    f_haupt = ImageFont.truetype(F_SERIF_BOLD, mm(15))
    for ln in wrap(d, titel, f_haupt, CONTENT_W):
        d.text((mm(20), y), ln, font=f_haupt, fill=EL)
        y += mm(18)
    y += mm(6)
    ...
    box_y = mm(255)
    box_h2 = mm(20)
    d.rounded_rectangle((mm(20), box_y, W - mm(20), box_y + box_h2), radius=mm(3), fill=EL_LIGHT)
    f_status_l = ImageFont.truetype(F_SANS_BOLD, mm(5.5))
    f_status = ImageFont.truetype(F_SANS_REG, mm(5))
    d.text((mm(28), box_y + mm(5)), "STAND DIESER AUSGABE", font=f_status_l, fill=EL)
...
SETS = [
    ("EL-AT", "KLARTEXT_EL-Zusatzblock_Autismus.pdf",
     "EL-Zusatzblock: Eltern autistischer Kinder",
     "7 Ergänzungskarten zur EL-Basis (KLARTEXT_EL-Deck_Basis_komplett.pdf, siehe dort für "
     "Anleitung, Nutzungsarten und Grenzen). Andockt an das AT-Kind-Deck – gedacht für Eltern, "
     "deren Kind mit dem AT-Deck begleitet wird.",
     "Diese Karten fokussieren bewusst auf die emotionale Erfahrung der Eltern, nicht auf "
     "Diagnostik oder Interventionsempfehlungen. Vor Veröffentlichung fachlich gegenlesen lassen.",
     "7 Karten vollständig. Fachliches Gegenlesen vor Veröffentlichung noch ausstehend."),
    ("EL-ADHS", "KLARTEXT_EL-Zusatzblock_ADHS.pdf",
     "EL-Zusatzblock: Eltern von Kindern mit ADHS",
     "7 Ergänzungskarten zur EL-Basis (KLARTEXT_EL-Deck_Basis_komplett.pdf, siehe dort für "
     "Anleitung, Nutzungsarten und Grenzen). Andockt an das geplante ADHS-Kind-Deck.",
     "Fokus auf die emotionale Erfahrung der Eltern. Die Karte zu Unterstützung/Behandlung "
     "(EL-ADHS-06) ist bewusst wertungsfrei zu Medikation formuliert. Vor Veröffentlichung "
     "fachlich gegenlesen lassen.",
     "7 Karten vollständig. Fachliches Gegenlesen vor Veröffentlichung noch ausstehend."),
    ("EL-PF", "KLARTEXT_EL-Zusatzblock_Pflegekinder.pdf",
     "EL-Zusatzblock: Pflegeeltern",
     "7 Ergänzungskarten zur EL-Basis (KLARTEXT_EL-Deck_Basis_komplett.pdf, siehe dort für "
     "Anleitung, Nutzungsarten und Grenzen). Andockt an das geplante Pflegekinder-Ergänzungsset.",
    ...
    ("EL-JD", "KLARTEXT_EL-Zusatzblock_Jugendliche.pdf",
     "EL-Zusatzblock: Eltern von Jugendlichen",
     "7 Ergänzungskarten zur EL-Basis (KLARTEXT_EL-Deck_Basis_komplett.pdf, siehe dort für "
     "Anleitung, Nutzungsarten und Grenzen). Andockt an das JD-Deck (Jugendliche).",
     "Allgemeine elterliche Erfahrungswerte der Pubertät/Ablösephase, keine diagnosespezifischen "
     "Inhalte – kein externer Fachprüfungs-Vorbehalt nötig.",
     "7 Karten vollständig, kein Gegenlesen-Vorbehalt."),
]
