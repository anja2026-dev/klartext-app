...
# -*- coding: utf-8 -*-
"""
KLARTEXT AT-Deck – Kartengenerator, adaptiert von build_card_lk.py. Fachprüfung durch externe
Autismus-Fachperson abgeschlossen (27.07.2026) – kein Entwurfs-/Fachprüfungs-Vorbehalt mehr.
Anders als EL/LK/TR: Kind-facing (wie JD/KD) statt Erwachsenen-Selbstreflexion – daher "TIPP FÜR DIE
INGRA" statt "TIPP FÜR DICH", KEINE dritte systemische Frage (Vorhersehbarkeit hat hier Vorrang,
siehe AT_Kartenkonzept_Entwurf.md), Rückseite immer exakt dieselbe Struktur.
"""
...
# ── Farben (AT-Deckfarbe: Salbeigrau #7C8C7E – kollisionsgeprüft gegen alle bestehenden App- und
# Deckfarben, deutlich desaturierter/heller als alle vorhandenen Grüntöne (M2 #3B8A68, M5 #4A6B4A,
# KD-Grün #2E9E5A). Bewusst ruhig/zurückhaltend gewählt, passend zum reizarmen Grundprinzip.) ──
AT          = (124, 140, 126)     # #7C8C7E
AT_LIGHT    = (237, 241, 236)     # #EDF1EC
AT_BORDER   = (201, 211, 201)     # #C9D3C9
...
def build_front(card, image_path, out_path):
    ...
    kopf_h = mm(11)
    d.rectangle((0, 0, CARD_W, kopf_h), fill=AT)
    logo = logo_mark(7)
    img.paste(logo, (mm(5), (kopf_h - logo.height) // 2), logo)
    f_badge = font(F_SANS_BOLD, 2.6)
    badge_text = card.get("badge", "AT · AUTISMUS-DECK")
    ...
    if os.path.exists(image_path):
        ...
        d.rectangle((0, bild_top, CARD_W, bild_top + bild_h), fill=(228, 233, 228))

    tf_top = bild_top + bild_h
    d.rectangle((0, tf_top, CARD_W, CARD_H), fill=AT_LIGHT)
    d.line((0, tf_top, CARD_W, tf_top), fill=AT_BORDER, width=mm(0.5))
    f_titel = font(F_SERIF_BOLD, 5)
    lines = wrap_text(d, card["titel"], f_titel, CARD_W - mm(10))
    ly = tf_top + mm(4)
    for ln in lines[:2]:
        d.text((mm(5), ly), ln, font=f_titel, fill=AT)
        ly += mm(6.2)
...
def build_back(card, out_path):
    ...
    nr_text = card.get("id_text", f"AT-{card['nr']:02d}")
    ...
    if len(titel_lines) > 2:
        ...
        for ln in titel_lines[:2]:
            d.text((pad, ty), ln, font=f_rstitel, fill=AT)
            ty += mm(5.6)
        ...
        y = max(ty, y + mm(9))
    d.line((pad, y, CARD_W - pad, y), fill=AT_BORDER, width=mm(0.4))
    y += mm(3.3)

    f_label = font(F_SANS_BOLD, 2.4)
    f_anleitung = font(F_SERIF_ITALIC, 3.1)
    d.text((pad, y), "ANLEITUNG", font=f_label, fill=AT)
    y += mm(4.3)
    ...
    d.text((pad, y), "FRAGEN", font=f_label, fill=AT)
    y += mm(4.6)
    ...
    for frage in card["fragen"]:
        ...
        rounded_rect(d, (pad, y, CARD_W - pad, y + box_h), radius=mm(2), fill=AT_LIGHT)
        d.rectangle((pad, y, pad + mm(1), y + box_h), fill=AT)
        ty = y + box_pad_y
        for ln in lines:
    ...
    d.line((pad, foot_y, CARD_W - pad, foot_y), fill=AT_BORDER, width=mm(0.4))
    logo = logo_mark(5.5)
    img.paste(logo, (pad, foot_y + mm(2)), logo)
    f_foot = font(F_SANS_REG, 2.3)
    footer_deck = card.get("footer_deck", "AT-Deck")
