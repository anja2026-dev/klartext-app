#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Baut die druckfertige PDF aus allen fertigen KD-Karten (30 von 30) + Bonus-Karte."""
from PIL import Image, ImageDraw, ImageFont
import os, sys
Image.init()
sys.path.insert(0, os.path.dirname(__file__))
from build_booklet_kd import (anleitung_seite1, anleitung_seite2, methodik_seite, glossar_seite,
                               quellen_seite, GLOSSAR)

KARTEN_DIR = "/sessions/kind-beautiful-ptolemy/mnt/outputs/kd_karten_komplett/"
OUT_PDF = "/sessions/kind-beautiful-ptolemy/mnt/outputs/KLARTEXT_KD-Deck_komplett.pdf"
...
def build_cover():
    ...
    d.text((mm(48), mm(34)), "Systemische Coaching-Impulskarten", font=f_sub, fill=(230, 245, 235))

    f_haupt = ImageFont.truetype(F_SERIF_BOLD, mm(34))
    d.text((mm(20), mm(90)), "KD-Deck", font=f_haupt, fill=KD)

    f_haupt2 = ImageFont.truetype(F_SANS_BOLD, mm(7))
    d.text((mm(20), mm(140)), "35 Coaching-Impulskarten für Grundschulkinder", font=f_haupt2, fill=KT_INK)

    f_themen_label = ImageFont.truetype(F_SANS_BOLD, mm(5.5))
    d.text((mm(20), mm(158)), "THEMENBEREICHE", font=f_themen_label, fill=KD)

    themen = ["Gefühle erkennen & benennen", "Wut, Angst, Trauer – was hilft mir",
              "Streit & Wiedergutmachung", "Mut & neue Situationen",
              "Freundschaft & Ausgrenzung", "Körper & Wohlfühlen",
    ...
    for i, t in enumerate(themen):
        x = mm(20)
        y = mm(168) + i * row_h
        d.ellipse((x, y + mm(2.2), x + mm(2.6), y + mm(4.8)), fill=KD)
        d.text((x + mm(6), y), t, font=f_thema, fill=KT_INK)

    box_y = mm(240)
    box_h = mm(22)
    d.rounded_rectangle((mm(20), box_y, W - mm(20), box_y + box_h), radius=mm(3), fill=(231, 245, 236))
    f_status_l = ImageFont.truetype(F_SANS_BOLD, mm(6))
    f_status = ImageFont.truetype(F_SANS_REG, mm(6))
    d.text((mm(28), box_y + mm(6)), "STAND DIESER AUSGABE", font=f_status_l, fill=KD)
    d.text((mm(28), box_y + mm(13)), "Alle 35 Karten vollständig, inkl. Bonus-Barometer-Karte.",
...
def run():
    ...
    bonus_vorn = os.path.join(KARTEN_DIR, "KD-Bonus_Vorderseite.png")
    bonus_hinten = os.path.join(KARTEN_DIR, "KD-Bonus_Rueckseite.png")
    if os.path.exists(bonus_vorn) and os.path.exists(bonus_hinten):
        pages.append(Image.open(bonus_vorn).convert("RGB"))
        pages.append(Image.open(bonus_hinten).convert("RGB"))

    for nr in range(1, 36):
        vorn = os.path.join(KARTEN_DIR, f"KD-{nr:02d}_Vorderseite.png")
        hinten = os.path.join(KARTEN_DIR, f"KD-{nr:02d}_Rueckseite.png")
