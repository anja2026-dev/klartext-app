...
# -*- coding: utf-8 -*-
"""
KLARTEXT KD-Deck – Kartengenerator, neu aufgebaut (27.07.2026) analog zu build_card_fs.py, da die
ursprünglichen KD-Kartendateien nie ins klartext-app-Repo kopiert wurden (nur ein alter Entwurf lag
noch im Scratch-Ordner). Brainy als durchgängige Coach-Figur nur im Bild (keine Textreferenz im
Renderer nötig). "TIPP FÜR DIE INGRA", KEINE dritte systemische Frage (Content-Achse, wie
JD/FS/AT/ADHS).
"""
...
# ── Farben (KD-Deckfarbe: KD-Grün #2E9E5A – bestehende, etablierte Farbe aus der frühen
# Projektphase, seitdem als Referenzfarbe in AT/TR-Kollisionschecks verwendet) ──
KD          = (46, 158, 90)       # #2E9E5A
KD_LIGHT    = (232, 247, 238)     # #E8F7EE
KD_BORDER   = (195, 225, 205)     # #C3E1CD
...
def build_front(card, image_path, out_path):
    ...
    badge_text = card.get("badge", "KD-DECK")
    ...
    if os.path.exists(image_path):
        ...
        d.rectangle((0, bild_top, CARD_W, bild_top + bild_h), fill=(220, 238, 227))

    tf_top = bild_top + bild_h
    d.rectangle((0, tf_top, CARD_W, CARD_H), fill=KD_LIGHT)
    d.line((0, tf_top, CARD_W, tf_top), fill=KD_BORDER, width=mm(0.5))
    f_titel = font(F_SERIF_BOLD, 5)
    lines = wrap_text(d, card["titel"], f_titel, CARD_W - mm(10))
    ly = tf_top + mm(4)
    for ln in lines[:2]:
        d.text((mm(5), ly), ln, font=f_titel, fill=KD)
        ly += mm(6.2)
...
def build_back(card, out_path):
    ...
    nr_text = card.get("id_text", f"KD-{card['nr']:02d}")
    ...
    if len(titel_lines) > 2:
        ...
        for ln in titel_lines[:2]:
            d.text((pad, ty), ln, font=f_rstitel, fill=KD)
            ty += mm(5.6)
    ...
    d.text((pad, y), "FRAGEN", font=f_label, fill=KD)
    y += mm(4.6)
    ...
    for frage in card["fragen"]:
        ...
        rounded_rect(d, (pad, y, CARD_W - pad, y + box_h), radius=mm(2), fill=KD_LIGHT)
        d.rectangle((pad, y, pad + mm(1), y + box_h), fill=KD)
        ty = y + box_pad_y
        for ln in lines:
    ...
    footer_deck = card.get("footer_deck", "KD-Deck")
