...
# -*- coding: utf-8 -*-
"""
KLARTEXT DaZ-GS-Deck (Deutsch als Zweitsprache, Grundschule) – Kartengenerator, adaptiert von
build_card_fs.py. "TIPP FÜR DIE INGRA", KEINE dritte systemische Frage – wie KD/FS/AT/ADHS. Kein
Fachprüfungs-Vorbehalt (Anjas eigene DaZ-/Traumapädagogik-Qualifikation deckt das ab, analog zu
FS/Pflegekinder), daher normaler footer_deck ohne "(Entwurf)".
"""
...
# ── Farben (DaZ-GS-Deckfarbe: Azurblau #00ACD6 – rechnerisch kollisionsgeprüft per RGB-Distanz
# gegen alle 22 bestehenden App-/Deck-/Modulfarben, Abstand 77.2 zur naechsten Farbe (App-LK-Blau),
# groesster Sicherheitsabstand der ganzen Serie, siehe DAZ-GS_Kartenkonzept_Entwurf.md) ──
DAZGS       = (0, 172, 214)       # #00ACD6
DAZGS_LIGHT = (229, 246, 250)     # #E5F6FA
DAZGS_BORDER= (173, 228, 241)     # #ADE4F1
DAZGS_TEXT  = (0, 94, 117)        # #005E75 – dunklere Textvariante fuer Lesbarkeit auf Weiss
...
def build_front(card, image_path, out_path):
    ...
    kopf_h = mm(11)
    d.rectangle((0, 0, CARD_W, kopf_h), fill=DAZGS)
    logo = logo_mark(7)
    img.paste(logo, (mm(5), (kopf_h - logo.height) // 2), logo)
    f_badge = font(F_SANS_BOLD, 2.6)
    badge_text = card.get("badge", "DAZ-GS · SPRACHE & ANKOMMEN")
    ...
    if os.path.exists(image_path):
        ...
        d.rectangle((0, bild_top, CARD_W, bild_top + bild_h), fill=(215, 240, 247))

    tf_top = bild_top + bild_h
    d.rectangle((0, tf_top, CARD_W, CARD_H), fill=DAZGS_LIGHT)
    d.line((0, tf_top, CARD_W, tf_top), fill=DAZGS_BORDER, width=mm(0.5))
    f_titel = font(F_SERIF_BOLD, 5)
    lines = wrap_text(d, card["titel"], f_titel, CARD_W - mm(10))
    ly = tf_top + mm(4)
    for ln in lines[:2]:
        d.text((mm(5), ly), ln, font=f_titel, fill=DAZGS_TEXT)
        ly += mm(6.2)
...
def build_back(card, out_path):
    ...
    nr_text = card.get("id_text", f"DAZ-GS-{card['nr']:02d}")
    ...
    if len(titel_lines) > 2:
        ...
        for ln in titel_lines[:2]:
            d.text((pad, ty), ln, font=f_rstitel, fill=DAZGS_TEXT)
            ty += mm(5.6)
        ...
        y = max(ty, y + mm(9))
    d.line((pad, y, CARD_W - pad, y), fill=DAZGS_BORDER, width=mm(0.4))
    y += mm(3.3)

    f_label = font(F_SANS_BOLD, 2.4)
    f_anleitung = font(F_SERIF_ITALIC, 3.1)
    d.text((pad, y), "ANLEITUNG", font=f_label, fill=DAZGS_TEXT)
    y += mm(4.3)
    ...
    d.text((pad, y), "FRAGEN", font=f_label, fill=DAZGS_TEXT)
    y += mm(4.6)
    ...
    for frage in card["fragen"]:
        ...
        rounded_rect(d, (pad, y, CARD_W - pad, y + box_h), radius=mm(2), fill=DAZGS_LIGHT)
        d.rectangle((pad, y, pad + mm(1), y + box_h), fill=DAZGS)
        ty = y + box_pad_y
        for ln in lines:
    ...
    d.line((pad, foot_y, CARD_W - pad, foot_y), fill=DAZGS_BORDER, width=mm(0.4))
    logo = logo_mark(5.5)
    img.paste(logo, (pad, foot_y + mm(2)), logo)
    f_foot = font(F_SANS_REG, 2.3)
    footer_deck = card.get("footer_deck", "DaZ-GS-Deck")
