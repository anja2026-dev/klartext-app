...
# -*- coding: utf-8 -*-
"""
KLARTEXT EL-Deck (Basis) – Kartengenerator (adaptiert von build_card_kd.py).
Baut Vorder- und Rückseite einer Karte als druckfertiges PNG (300 dpi, A6 105x148mm).
Unterschied zu JD/KD: Rückseiten-Hinweisbox heißt "TIPP FÜR DICH" (nicht "TIPP FÜR DIE INGRA"),
da sich das EL-Deck direkt an Eltern richtet (Dual-Use: allein oder mit Begleitung nutzbar).
"""
...
# ── Farben (EL-Deckfarbe: Terracotta #BF5B3E – kollisionsgeprüft gegen bestehende
# App-Modulfarben: KD-App-Modul=Teal #00838F/#4FC3C7, Elternkurs(App)=Magenta #AD1457,
# LK-App-Modul=Blau #1565C0, TR-App-Modul=Braun/Gold #7A4C00. Terracotta ist frei.) ──
EL          = (191, 91, 62)       # #BF5B3E
EL_LIGHT    = (250, 235, 227)     # #FAEBE3
EL_BORDER   = (232, 196, 176)     # #E8C4B0
KT_PRIMARY  = (27, 58, 75)        # #1B3A4B
KT_ACCENT   = (110, 198, 160)     # #6EC6A0
KT_INK      = (45, 45, 45)        # #2D2D2D
KT_MUTED    = (122, 112, 96)      # #7A7060
WHITE       = (255, 255, 255)
KT_PAPER    = (245, 240, 232)     # #F5F0E8 – für Tipp-Box, konsistent mit JD/KD
HINWEIS_BORDER = (196, 176, 120)

# ── Fonts (gleiche Platzhalter wie JD/KD) ──
...
def build_front(card, image_path, out_path):
    ...
    if os.path.exists(image_path):
        ...
        d.rectangle((0, bild_top, CARD_W, bild_top + bild_h), fill=(230, 224, 240))

    tf_top = bild_top + bild_h
    d.rectangle((0, tf_top, CARD_W, CARD_H), fill=EL_LIGHT)
    d.line((0, tf_top, CARD_W, tf_top), fill=EL_BORDER, width=mm(0.5))
    f_titel = font(F_SERIF_BOLD, 5)
    lines = wrap_text(d, card["titel"], f_titel, CARD_W - mm(10))
    ly = tf_top + mm(4)
    for ln in lines[:2]:
        d.text((mm(5), ly), ln, font=f_titel, fill=EL)
        ly += mm(6.2)
...
def build_back(card, out_path):
    ...
    nr_text = card.get("id_text", f"EL-{card['nr']:02d}")
    ...
    if len(titel_lines) > 2:
        # bei sehr langen Titeln: ID-Kennung in eigene Zeile verschieben statt weiter zu quetschen
        titel_lines = wrap_text(d, card["titel"], f_rstitel, CARD_W - 2 * pad)
        ty = y
        for ln in titel_lines[:2]:
            d.text((pad, ty), ln, font=f_rstitel, fill=EL)
            ty += mm(5.6)
        ...
        y = max(ty, y + mm(9))
    d.line((pad, y, CARD_W - pad, y), fill=EL_BORDER, width=mm(0.4))
    y += mm(3.3)

    f_label = font(F_SANS_BOLD, 2.4)
    f_anleitung = font(F_SERIF_ITALIC, 3.1)
    d.text((pad, y), "ANLEITUNG", font=f_label, fill=EL)
    y += mm(4.3)
    ...
    d.text((pad, y), "IMPULSFRAGEN", font=f_label, fill=EL)
    y += mm(4.6)
    ...
    for frage in card["fragen"]:
        ...
        rounded_rect(d, (pad, y, CARD_W - pad, y + box_h), radius=mm(2), fill=EL_LIGHT)
        d.rectangle((pad, y, pad + mm(1), y + box_h), fill=EL)
        ty = y + box_pad_y
        for ln in lines:
    ...
    if card.get("systemfrage"):
        ...
        SYS = (27, 58, 75)         # KT_PRIMARY, navy – markiert die Technik-Frage klar anders als die Terracotta-Boxen
    ...
    d.line((pad, foot_y, CARD_W - pad, foot_y), fill=EL_BORDER, width=mm(0.4))
    logo = logo_mark(5.5)
    img.paste(logo, (pad, foot_y + mm(2)), logo)
    f_foot = font(F_SANS_REG, 2.3)
    footer_deck = card.get("footer_deck", "EL-Deck")
...
def side_by_side_preview(front, back, out_path, gap=30):
    w = front.width + back.width + gap
    h = max(front.height, back.height) + gap
