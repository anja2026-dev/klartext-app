...
# -*- coding: utf-8 -*-
"""
KLARTEXT JD-Deck – Kartengenerator, neu aufgebaut (27.07.2026) analog zu build_card_at.py, da die
ursprünglichen JD-Kartendateien nie ins klartext-app-Repo kopiert wurden (nur build_booklet.py
existierte schon). Kein Brainy im Bild (wie AT/ADHS/TR), "TIPP FÜR DIE INGRA", KEINE dritte
systemische Frage (Content-Achse, wie KD/FS/AT/ADHS).
"""
...
# ── Farben (JD-Deckfarbe: JD-Petrol #2F6B6E – bestehende, etablierte Farbe aus der frühen
# Projektphase, seitdem als Referenzfarbe in AT/TR-Kollisionschecks verwendet) ──
JD          = (47, 107, 110)      # #2F6B6E
JD_LIGHT    = (234, 243, 243)     # #EAF3F3
JD_BORDER   = (195, 220, 220)     # #C3DCDC
...
def build_front(card, image_path, out_path):
    ...
    if os.path.exists(image_path):
        ...
        d.rectangle((0, bild_top, CARD_W, bild_top + bild_h), fill=(220, 232, 232))

    tf_top = bild_top + bild_h
    d.rectangle((0, tf_top, CARD_W, CARD_H), fill=JD_LIGHT)
    d.line((0, tf_top, CARD_W, tf_top), fill=JD_BORDER, width=mm(0.5))
    f_titel = font(F_SERIF_BOLD, 5)
    lines = wrap_text(d, card["titel"], f_titel, CARD_W - mm(10))
    ly = tf_top + mm(4)
    for ln in lines[:2]:
        d.text((mm(5), ly), ln, font=f_titel, fill=JD)
        ly += mm(6.2)
...
def build_back(card, out_path):
    ...
    nr_text = card.get("id_text", f"JD-{card['nr']:02d}")
    ...
    if len(titel_lines) > 2:
        ...
        for ln in titel_lines[:2]:
            d.text((pad, ty), ln, font=f_rstitel, fill=JD)
            ty += mm(5.6)
        ...
        y = max(ty, y + mm(9))
    d.line((pad, y, CARD_W - pad, y), fill=JD_BORDER, width=mm(0.4))
    y += mm(3.3)

    f_label = font(F_SANS_BOLD, 2.4)
    f_anleitung = font(F_SERIF_ITALIC, 3.1)
    d.text((pad, y), "ANLEITUNG", font=f_label, fill=JD)
    y += mm(4.3)
    ...
    d.text((pad, y), "FRAGEN", font=f_label, fill=JD)
    y += mm(4.6)
    ...
    for frage in card["fragen"]:
        ...
        rounded_rect(d, (pad, y, CARD_W - pad, y + box_h), radius=mm(2), fill=JD_LIGHT)
        d.rectangle((pad, y, pad + mm(1), y + box_h), fill=JD)
        ty = y + box_pad_y
        for ln in lines:
    ...
    d.line((pad, foot_y, CARD_W - pad, foot_y), fill=JD_BORDER, width=mm(0.4))
    logo = logo_mark(5.5)
    img.paste(logo, (pad, foot_y + mm(2)), logo)
    f_foot = font(F_SANS_REG, 2.3)
    footer_deck = card.get("footer_deck", "JD-Deck")
