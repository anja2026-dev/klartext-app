...
# -*- coding: utf-8 -*-
"""
KLARTEXT LK-Deck (Basis) – Kartengenerator (adaptiert von build_card_el.py).
Baut Vorder- und Rückseite einer Karte als druckfertiges PNG (300 dpi, A6 105x148mm).
Rückseiten-Hinweisbox "TIPP FÜR DICH" wie bei EL, dritte systemische Frage wie bei EL.
"""
...
# ── Farben (LK-Deckfarbe: Pflaume/Aubergine #6B4E71 – kollisionsgeprüft gegen bestehende
# App-Modulfarben: LK-App-Modul=Blau #1565C0, KD-App-Modul=Teal #00838F/#4FC3C7,
# Elternkurs(App)=Magenta #AD1457, TR-App-Modul=Braun/Gold #7A4C00, EL-Deck=Terracotta #BF5B3E.
# Pflaume ist frei und bewusst weit weg vom App-Blau, um Verwechslung mit LK-01-17 zu vermeiden.) ──
LK          = (107, 78, 113)      # #6B4E71
LK_LIGHT    = (238, 231, 239)     # #EEE7EF
LK_BORDER   = (206, 189, 208)     # #CEBDD0
...
def build_front(card, image_path, out_path):
    ...
    kopf_h = mm(11)
    d.rectangle((0, 0, CARD_W, kopf_h), fill=LK)
    logo = logo_mark(7)
    img.paste(logo, (mm(5), (kopf_h - logo.height) // 2), logo)
    f_badge = font(F_SANS_BOLD, 2.6)
    badge_text = card.get("badge", "LK · LEHRKRAFT-DECK")
    ...
    d.rectangle((0, tf_top, CARD_W, CARD_H), fill=LK_LIGHT)
    d.line((0, tf_top, CARD_W, tf_top), fill=LK_BORDER, width=mm(0.5))
    f_titel = font(F_SERIF_BOLD, 5)
    lines = wrap_text(d, card["titel"], f_titel, CARD_W - mm(10))
    ly = tf_top + mm(4)
    for ln in lines[:2]:
        d.text((mm(5), ly), ln, font=f_titel, fill=LK)
        ly += mm(6.2)
...
def build_back(card, out_path):
    ...
    nr_text = card.get("id_text", f"LK-R-{card['nr']:02d}")
    ...
    if len(titel_lines) > 2:
        ...
        for ln in titel_lines[:2]:
            d.text((pad, ty), ln, font=f_rstitel, fill=LK)
            ty += mm(5.6)
        ...
        y = max(ty, y + mm(9))
    d.line((pad, y, CARD_W - pad, y), fill=LK_BORDER, width=mm(0.4))
    y += mm(3.3)

    f_label = font(F_SANS_BOLD, 2.4)
    f_anleitung = font(F_SERIF_ITALIC, 3.1)
    d.text((pad, y), "ANLEITUNG", font=f_label, fill=LK)
    y += mm(4.3)
    ...
    d.text((pad, y), "IMPULSFRAGEN", font=f_label, fill=LK)
    y += mm(4.6)
    ...
    for frage in card["fragen"]:
        ...
        rounded_rect(d, (pad, y, CARD_W - pad, y + box_h), radius=mm(2), fill=LK_LIGHT)
        d.rectangle((pad, y, pad + mm(1), y + box_h), fill=LK)
        ty = y + box_pad_y
        for ln in lines:
    ...
    d.line((pad, foot_y, CARD_W - pad, foot_y), fill=LK_BORDER, width=mm(0.4))
    logo = logo_mark(5.5)
    img.paste(logo, (pad, foot_y + mm(2)), logo)
    f_foot = font(F_SANS_REG, 2.3)
    footer_deck = card.get("footer_deck", "LK-Deck")
