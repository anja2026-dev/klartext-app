...
# -*- coding: utf-8 -*-
"""
KLARTEXT FS-Deck (Förderschule) – Kartengenerator, adaptiert von build_card_at.py/adhs.py.
Sprachlich vereinfachte KD-Variante (kein Neuaufbau, siehe FS_Kartenkonzept_Entwurf.md), gleiche
Themen/Reihenfolge wie KD, "TIPP FÜR DIE INGRA", KEINE dritte systemische Frage – identisch zur
KD-Struktur. Kein Fachprüfungs-Vorbehalt (Anjas eigene 7 Jahre Förderschule-Praxiserfahrung deckt
das ab, analog zum Pflegekinder-Ergänzungsset), daher normaler footer_deck ohne "(Entwurf)".
"""
...
# ── Farben (FS-Deckfarbe: Sonnenblumengelb #DEB234 – rechnerisch kollisionsgeprüft per
# RGB-Distanz gegen alle bestehenden App-/Deck-/Modulfarben, größter Sicherheitsabstand aller
# geprüften Kandidaten, siehe FS_Kartenkonzept_Entwurf.md) ──
FS          = (222, 178, 52)      # #DEB234
FS_LIGHT    = (251, 243, 220)     # #FBF3DC
FS_BORDER   = (237, 219, 168)     # #EDDBA8
...
def build_front(card, image_path, out_path):
    ...
    kopf_h = mm(11)
    d.rectangle((0, 0, CARD_W, kopf_h), fill=FS)
    logo = logo_mark(7)
    img.paste(logo, (mm(5), (kopf_h - logo.height) // 2), logo)
    f_badge = font(F_SANS_BOLD, 2.6)
    badge_text = card.get("badge", "FS · FÖRDERSCHULE-DECK")
    ...
    if os.path.exists(image_path):
        ...
        d.rectangle((0, bild_top, CARD_W, bild_top + bild_h), fill=(240, 232, 210))

    tf_top = bild_top + bild_h
    d.rectangle((0, tf_top, CARD_W, CARD_H), fill=FS_LIGHT)
    d.line((0, tf_top, CARD_W, tf_top), fill=FS_BORDER, width=mm(0.5))
    f_titel = font(F_SERIF_BOLD, 5)
    lines = wrap_text(d, card["titel"], f_titel, CARD_W - mm(10))
    ly = tf_top + mm(4)
    for ln in lines[:2]:
        d.text((mm(5), ly), ln, font=f_titel, fill=(150, 112, 20))
        ly += mm(6.2)
...
def build_back(card, out_path):
    ...
    nr_text = card.get("id_text", f"FS-{card['nr']:02d}")
    ...
    if len(titel_lines) > 2:
        ...
        for ln in titel_lines[:2]:
            d.text((pad, ty), ln, font=f_rstitel, fill=(150, 112, 20))
            ty += mm(5.6)
        ...
        y = max(ty, y + mm(9))
    d.line((pad, y, CARD_W - pad, y), fill=FS_BORDER, width=mm(0.4))
    y += mm(3.3)

    f_label = font(F_SANS_BOLD, 2.4)
    f_anleitung = font(F_SERIF_ITALIC, 3.1)
    d.text((pad, y), "ANLEITUNG", font=f_label, fill=(150, 112, 20))
    y += mm(4.3)
    ...
    d.text((pad, y), "FRAGEN", font=f_label, fill=(150, 112, 20))
    y += mm(4.6)
    ...
    for frage in card["fragen"]:
        ...
        rounded_rect(d, (pad, y, CARD_W - pad, y + box_h), radius=mm(2), fill=FS_LIGHT)
        d.rectangle((pad, y, pad + mm(1), y + box_h), fill=FS)
        ty = y + box_pad_y
        for ln in lines:
    ...
    d.line((pad, foot_y, CARD_W - pad, foot_y), fill=FS_BORDER, width=mm(0.4))
    logo = logo_mark(5.5)
    img.paste(logo, (pad, foot_y + mm(2)), logo)
    f_foot = font(F_SANS_REG, 2.3)
    footer_deck = card.get("footer_deck", "FS-Deck")
