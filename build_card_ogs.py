...
# -*- coding: utf-8 -*-
"""
KLARTEXT OGS-Deck (Basis) – Kartengenerator (adaptiert von build_card_lk.py).
Baut Vorder- und Rückseite einer Karte als druckfertiges PNG (300 dpi, A6 105x148mm).
Reflexionsformat wie LK-Basis/EL-Basis: Anleitung + Impulsfragen + 3. systemische Frage + Tipp für dich.
"""
...
# ── Farben (OGS-Deckfarbe: Limette/Frühlingsgrün #8BC34A – rechnerisch kollisionsgeprüft per
# RGB-Distanz gegen alle bestehenden Deck- und Insel-Farben (JD-Petrol, KD-Grün, EL-Terracotta,
# LK-Pflaume, TR-Slate-Blau, FS-Sonnenblumengelb, AT-Salbeigrau, ADHS-Periwinkle, DaZGS-Azurblau,
# DaZSek1-Bordeaux, TK-DeepPurple, alle 9 Insel-Farben). Nächste Nachbarfarbe: AT-Salbeigrau,
# Distanz 77.2 – ausreichend abgesetzt (Referenzwert im Projekt üblich: >70).) ──
OGS         = (139, 195, 74)      # #8BC34A
OGS_LIGHT   = (238, 246, 227)     # #EEF6E3
OGS_BORDER  = (206, 227, 176)     # #CEE3B0
...
def build_front(card, image_path, out_path):
    ...
    if os.path.exists(image_path):
        ...
        d.rectangle((0, bild_top, CARD_W, bild_top + bild_h), fill=OGS_LIGHT)

    tf_top = bild_top + bild_h
    d.rectangle((0, tf_top, CARD_W, CARD_H), fill=OGS_LIGHT)
    d.line((0, tf_top, CARD_W, tf_top), fill=OGS_BORDER, width=mm(0.5))
    f_titel = font(F_SERIF_BOLD, 5)
    lines = wrap_text(d, card["titel"], f_titel, CARD_W - mm(10))
    ly = tf_top + mm(4)
    for ln in lines[:2]:
        d.text((mm(5), ly), ln, font=f_titel, fill=OGS)
        ly += mm(6.2)
...
def build_back(card, out_path):
    ...
    nr_text = card.get("id_text", f"OGS-{card['nr']:02d}")
    ...
    if len(titel_lines) > 2:
        ...
        for ln in titel_lines[:2]:
            d.text((pad, ty), ln, font=f_rstitel, fill=OGS)
            ty += mm(5.6)
        ...
        y = max(ty, y + mm(9))
    d.line((pad, y, CARD_W - pad, y), fill=OGS_BORDER, width=mm(0.4))
    y += mm(3.3)

    f_label = font(F_SANS_BOLD, 2.4)
    f_anleitung = font(F_SERIF_ITALIC, 3.1)
    d.text((pad, y), "ANLEITUNG", font=f_label, fill=OGS)
    y += mm(4.3)
    ...
    d.text((pad, y), "IMPULSFRAGEN", font=f_label, fill=OGS)
    y += mm(4.6)
    ...
    for frage in card["fragen"]:
        ...
        rounded_rect(d, (pad, y, CARD_W - pad, y + box_h), radius=mm(2), fill=OGS_LIGHT)
        d.rectangle((pad, y, pad + mm(1), y + box_h), fill=OGS)
        ty = y + box_pad_y
        for ln in lines:
    ...
    d.line((pad, foot_y, CARD_W - pad, foot_y), fill=OGS_BORDER, width=mm(0.4))
    logo = logo_mark(5.5)
    img.paste(logo, (pad, foot_y + mm(2)), logo)
    f_foot = font(F_SANS_REG, 2.3)
    footer_deck = card.get("footer_deck", "OGS-Deck")
